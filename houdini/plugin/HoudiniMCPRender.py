...
def _check_viewport_ready():
    """
    Pre-flight check for any viewport-based capture (flipbook, OpenGL render).
    Returns (ok: bool, message: str). When ok=False, the message is a clear,
    user-actionable explanation — return it as the error to the MCP client
    instead of letting Houdini trigger a fatal OpenGL dialog and hang the
    socket.
    """
    ...
    if sv is None:
        return False, (
            ...
            "(Windows -> Scene View) and try again."
        )
    try:
        if hou.updateMode() == hou.updateMode.Manual:
            return False, (
                "Houdini is in Manual update mode. Viewport rendering will "
                "trigger a fatal OpenGL 3.3 dialog and hang the MCP call. "
                "Switch to Auto Update: in Python Shell run "
                "`hou.setUpdateMode(hou.updateMode.AutoUpdate)`, or in the UI "
                "top-right of Scene View -> Update -> Auto Update."
            )
    except Exception:
        pass  # if probe itself fails, proceed and let the render fail naturally
    return True, ""
