...
| 1 | Diagnosis + shared plumbing | **done** 2026-08-12 — live-verified on local 20.5.278. pc137: Sashok started `hmcp` there (RDP), `check_contract.py` confirmed live against `10.10.10.31` — **33/33 commands agree**. No full MCP round-trip through Claude Code against pc137 this session (would need `HMCP_HOST` repointed + a Claude Code restart, judged unnecessary once the raw-socket contract check passed) — the `houdini2` bridge tools in this session still address local Houdini, not pc137 |
