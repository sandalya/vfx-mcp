...
def cmd_get_nodes_in_view(payload):
    ...
    pan/zoom. See little_helpers.nuke_utils.dag_viewport_rect for how the
    viewport rect itself is reconstructed.
    """
    from little_helpers.nuke_utils import dag_viewport_rect
    center, zoom, (w_px, h_px), vis_rect = dag_viewport_rect()
...
def cmd_list_render_dir(payload):
    """Thin MCP adapter over little_helpers.nuke_utils.list_render_dir."""
    from little_helpers.nuke_utils import list_render_dir
    return list_render_dir(payload.get("path"))
...
MENU_PATH = "MCP/Server/Unused/Quick Print Test"
MCP_MENU_PATH = "MCP/Server/MCP Server"
...
def register_menu():
    ...
    menu.addCommand(
        ...
        # picker in little_helpers. Still reachable from the menu.
    )
