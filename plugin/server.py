...
class HoudiniMCPServer:
    ...
    def _execute_command_internal(self, command):
        ...
        # --- HoudiniMCP audit log (patched) ---
        try:
            import os as _os, datetime as _dt, json as _json
            _log_path = _os.path.join(_os.path.expanduser('~'), 'houdini_mcp_audit.log')
            with open(_log_path, 'a', encoding='utf-8') as _f:
                _f.write('[' + _dt.datetime.now().isoformat(timespec='seconds') + '] ' + str(cmd_type) + ' ' + _json.dumps(params, default=str)[:500] + '\n')
        except Exception:
            pass
        # --- end audit log ---
        ...
        handlers = {
            ...
            "set_node_parameter": self.set_node_parameter,
    ...
    def get_scene_info(self, max_nodes=100, context_filter=None):
        """
        Returns basic info about the current .hip file and a sample of top-level nodes.
        max_nodes: cap on returned nodes (default 100).
        context_filter: optional list of context names (e.g. ["obj"]) — defaults to all.
        """
        try:
    ...
    def set_node_parameter(self, path, parm_name, value):
        ...
        if not parm:
            raise ValueError(f"Node {path} has no parameter '{parm_name}'")
        old_value = parm.eval()
        parm.set(value)
        return {
            "path": node.path(),
