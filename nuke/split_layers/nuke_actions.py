...
def split_explicit(node, layers, alpha, unpremult_all, merge_all, postage_stamp, mirror):
    ...
    for i in layers[1:]:
        # Dot to right (x2 per Sashok's ask -- more breathing room between forked shuffle branches)
        current_node = nuke.nodes.Dot(xpos=prev_node.xpos() + (400*mirror), ypos=prev_node.ypos())
        current_node.connectInput(0, prev_node)
        fork_node = current_node
        prev_node = current_node
    ...
    if not merge_all:
        ...
        for n, i in enumerate(last_nodes):
            if n < len(last_nodes) - 1:
                # First merge (right below the shuffle row) x7 per Sashok's ask;
                # every merge-to-merge step after that back to the original 75.
