...
- `Nuke/Edit/Paste` — `Ctrl+V` — `with nuke.lastHitGroup(): nuke.nodePaste(nukescripts.cut_paste_file())`. **Never touched** by `little_helpers.repath` — see the next entry for why that's worth stating explicitly, given the history.
- `Nuke/Edit/Repath Paste` — `Alt+V` — `little_helpers.paste_and_maybe_repath`, a standalone `addCommand` at its own new menu path, added 2026-09-11/12. First version of this (2026-09-11) took the shortcut of *overriding* `Edit/Paste` itself (`findItem`/`removeItem`/`addCommand` on that exact path, rebinding its hotkey to `Alt+V`) — confirmed live 2026-09-12 that `removeItem` really does drop the native `Ctrl+V` binding for good, no fallback, so a second override (`Edit/Paste (Plain)`, a hand-copied reimplementation of the native paste body) had to be added just to give `Ctrl+V` anything to do again. Sashok caught the actual problem: `Alt+V` was free the whole time and never needed `Edit/Paste`'s slot at all. Redesigned same day to a plain standalone command instead — `Edit/Paste` is now never found/removed/touched by this package, so native `Ctrl+V` needs no restoration and can never drift from whatever Nuke's own Paste actually does, including across future Nuke versions.
