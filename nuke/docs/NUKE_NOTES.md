...
- `Nuke/Edit/Paste` — `Ctrl+V` — `with nuke.lastHitGroup(): nuke.nodePaste(nukescripts.cut_paste_file())`. **Never touched** by `little_helpers.repath` — see `Nodes/Little Helpers/Repath Paste` below for why that's worth stating explicitly, given the history.
- `Nodes/Little Helpers/Repath Paste` — `Alt+V` — `little_helpers.paste_and_maybe_repath`, a standalone `addCommand` alongside the package's other three tools, added 2026-09-11/12. Went through two iterations same-day: first (2026-09-11) took the shortcut of *overriding* `Nuke/Edit/Paste` itself (`findItem`/`removeItem`/`addCommand` on that exact path, rebinding its hotkey to `Alt+V`) — confirmed live that `removeItem` really does drop the native `Ctrl+V` binding for good, no fallback, so a second override (`Edit/Paste (Plain)`, a hand-copied reimplementation of the native paste body) had to be added just to give `Ctrl+V` anything to do again. Sashok caught the actual problem: `Alt+V` was free the whole time and never needed `Edit/Paste`'s slot at all, so `Edit/Paste` should just never be touched. Redesigned twice same day: first to a standalone command still parked under `Nuke/Edit/Repath Paste`, then moved again to sit with the other three under `Nodes/Little Helpers/...` once there was no remaining reason for it to live under `Edit` at all. `Edit/Paste` itself was never found/removed/touched by either redesign, so native `Ctrl+V` needs no restoration and can never drift from whatever Nuke's own Paste actually does, including across future Nuke versions.
- `Nuke/Edit/@;Paste2` — `Ctrl+Shift+V` — `nuke.nodePaste(nukescripts.cut_paste_file())` (no `lastHitGroup()` — pastes without regard to the group under the cursor). Not touched by `little_helpers.repath`.
